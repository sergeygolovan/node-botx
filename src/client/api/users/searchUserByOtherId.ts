...
export class BotXAPISearchUserByOtherIdRequestPayload extends UnverifiedPayloadBaseModel {
  other_id!: string;

  static fromDomain(otherId: string): BotXAPISearchUserByOtherIdRequestPayload {
    return new BotXAPISearchUserByOtherIdRequestPayload({
      other_id: otherId,
    });
  }
}
...
export class SearchUserByOtherIdMethod extends AuthorizedBotXMethod {
  constructor(
    senderBotId: string,
  ...
  async execute(payload: BotXAPISearchUserByOtherIdRequestPayload): Promise<BotXAPISearchUserResponsePayload> {
    const path = "/api/v3/botx/users/by_other_id";
